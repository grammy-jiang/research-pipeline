...
def _mock_config() -> MagicMock:
    """Build a mock PipelineConfig with conversion defaults."""
    config = MagicMock()
    config.conversion.backend = "docling"
    config.conversion.timeout_seconds = 300
    config.conversion.marker.force_ocr = False
    config.conversion.marker.use_llm = False
    config.conversion.marker.llm_service = ""
    config.conversion.marker.llm_api_key = ""
    return config
...
class TestNonPdfExtension:
    ...
    @patch(_ENSURE_BUILTINS)
    @patch(_LOAD_CONFIG, return_value=_mock_config())
    @patch(_GET_BACKEND)
...
class TestDoclingImportError:
    """Tests for missing backend dependency."""
