...
_ENV_PREFIX = "ARXIV_PAPER_PIPELINE_"
...
def _apply_env_overrides(data: dict) -> dict:  # type: ignore[type-arg]
    ...
    ``ARXIV_PAPER_PIPELINE_<SECTION>_<KEY>`` (uppercase, underscore-separated).

    Known overrides:
    - ARXIV_PAPER_PIPELINE_CONFIG: config file path (handled upstream)
    - ARXIV_PAPER_PIPELINE_CACHE_DIR: override cache directory
    - ARXIV_PAPER_PIPELINE_WORKSPACE: override workspace directory
    - ARXIV_PAPER_PIPELINE_ALLOW_LIVE: enable live arXiv access
    - ARXIV_PAPER_PIPELINE_DISABLE_LLM: force LLM off
    - ARXIV_PAPER_PIPELINE_LLM_PROFILE: LLM model selection
    """
    env_map = {
...
def load_config(config_path: Path | None = None) -> PipelineConfig:
    ...
    Args:
        config_path: Path to a TOML config file. If None, checks
            ``ARXIV_PAPER_PIPELINE_CONFIG`` env var, then ``config.toml``
