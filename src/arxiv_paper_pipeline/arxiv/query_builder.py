...
def build_query_from_plan(plan: QueryPlan) -> list[str]:
    ...
    if plan.must_terms:
        title_q = build_field_query("ti", plan.must_terms, "AND")
        abs_q = build_field_query("abs", plan.must_terms, "AND")
    ...
    if plan.must_terms and plan.nice_terms:
        combined = plan.must_terms + plan.nice_terms[:2]
        all_q = build_field_query("all", combined, "AND")
        q = all_q
        if plan.candidate_categories:
            cat_q = build_category_filter(plan.candidate_categories)
            q = f"({q}) AND ({cat_q})"
    ...
    # Variant 3: broader search with nice terms only
    if plan.nice_terms:
        nice_q = build_field_query("all", plan.nice_terms[:3], "OR")
        q = nice_q
        if plan.candidate_categories:
            cat_q = build_category_filter(plan.candidate_categories)
            q = f"({q}) AND ({cat_q})"
