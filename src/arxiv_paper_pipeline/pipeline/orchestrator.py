...
from arxiv_paper_pipeline.infra.clock import date_window, utc_now
...
def run_pipeline(
    ...
    if not (resume and _is_stage_complete(manifest, "search")):
        ...
        all_candidate_lists = []
        for q in queries:
            candidates, raw_paths = arxiv_client.search(
                query=q,
        ...
        candidates = dedup_across_queries(all_candidate_lists)
